...
onDomReady(function(){
  privacyBox = new PrivacyBox({onPrivacySave: onCustomPrivacySave, saveList: canSaveList()});
  each (['ppMenu', 'dpHead', 'dpMenu'], function(){appendDiv(this);});
});
...
function dpItems(pp_tag, toUp) {
  ...
  var k = [], r = {};
  each(friends_lists, function(i,v){k.push(i);});
...
function onCustomPrivacySave(pp_tag, friends, friends_info, list_id, list_name, minus_friends, minus_friends_info) {
 ...
 if (list_id) {
  ...
  ge('pp_'+pp_tag).innerHTML = options[pp_lists_id];
   if (canSaveList()) {
    privacyBox.setOptions({saveList: false});
  }
  ...
  if (!isArray(minus_friends)) {
    ...
    var onlyIds = true;
    if(isList){
      each(friends, function(i,v){if(v>0){onlyLists = false;}else{onlyIds = false; dp_checked[pp_tag][-v] = -v;}});
    }
    var option_id = isList ? (onlyLists? pp_lists_id : pp_friends_id) : friends;
    ge('pp_'+pp_tag).innerHTML = options[option_id];

    var count = friends.length, str = '';
    if(isList) {
      str = ': ' + friends_info.join(', ');
    ...
    debugLog(isList);
      if(isList && onlyLists){pp_selected[pp_tag] = pp_lists_id;}
      else if(isList && onlyIds){pp_selected[pp_tag] = pp_friends_id;}
      else if(!isList) {
...
window.showCustomBox = function(options) {
  ...
  if(!customMB[key]) {
    customMB[key] = new MessageBox({title:options.title || getLang('privacy_custom_title'), width:400, returnHidden: true});
    customMB[key].addButton({label:getLang('global_close'), style:'button_no', onClick:customMB[key].hide});
    customMB[key].addButton({label:getLang('global_save'), onClick:function(){
    ...
    '<div style="padding: 5px 0"><input type="hidden" id="minus_friends_list"/></div></div>');
      customMB[key].plus_flist_type = new DropdownMenu([{i:0,l:l[0]},{i:1,l:l[1]},{i:2,l:l[2]},/*{i:3,l:l[3]},*/{i:4,l:l[4]}], {target: ge('plus_friends_type_'+key), onSelect: function(e){
      ...
      customMB[key].plus_flist = new Selector(ge('plus_friends_list'), friendsPrivacyList, {defaultItems:def, width:350, placeholder:'������� ��� ����� ��� �������� ������ ������', onChange:function(v) {
        if(v==='')return;
        var changed = false;
        v = v.split(',');
        var items = [];
      ...
      }, selectedItems:plus, introText:'������� ����� ������, ������� �� ������ ��������� ������'});
      customMB[key].minus_flist = new Selector(ge('minus_friends_list'), friendsPrivacyList, {defaultItems:def, width:350, placeholder:'������� ��� ����� ��� �������� ������ ������', onChange:function(v){
      if(v==='')return;
        var changed = false;
        v = v.split(',');
        var items = [];
      ...
      }, selectedItems:minus, introText:'������� ����� ������, ������� �� ������ ��������� ������'});
    };

    if(!friendsPrivacyList) {
      Ajax.Send('friends_ajax.php', {act:'custom'}, {onSuccess:function(o,t){
