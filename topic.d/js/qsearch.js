...
var w = window;
  ...
  if (window.vk && vk.al) {
    var ajxPost = function (url, query, cb) {
      ajax.post(url, query, {onDone: cb, cache: 1});
    };
  } else {
    var hintsCache = {};
    var ajxPost = function (url, query, cb) {
      var cacheKey = url + '#' + ajx2q(query);
      if (hintsCache[cacheKey]) {
        cb.apply(null, hintsCache[cacheKey]);
        return;
      }
      Ajax.Send(url, query, {onSuccess: function (o, t) {
        var res = eval('(' + t + ')');
        hintsCache[cacheKey] = res;
        cb.apply(null, hintsCache[cacheKey]);
      }});
    };
  }
...
self.searchFunc = function(inputStr, array, key, showCount, caseSensitive, lat, startWords) {
  ...
  var defaultEqual = function(regExp) {
    replaceStr = keyStr.replace(/�/gi, '�').replace(regExp, rChars);
    return (keyStr != replaceStr);
  }
  var startWordsEqual = function(regExp) {
    keyStr = keyStr.replace(/�/gi, '�').split(/\s/);
...
var hints = {
  get: function() {
    ...
    if (hints.val) {
      ...
      ajxPost('hints.php', {act: 'a_hints', q: hints.val, section: ''}, hints.show);
    }
  },
...
try{jsDispatcher.triggerOnload('qsearch');}catch(e){}
