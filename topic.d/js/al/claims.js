...
claimContent: function(cid, type, owner_id, id, hash) {
  ...
  ajax.post('/claims', {act: 'a_claim', claim_id: cid, type: type, id: id, owner_id: owner_id, hash: hash}, {
    onDone: function(response) {
      ge('claim' + cid + type + owner_id + '_' + id).innerHTML = '<a href="#" onclick="Claims.unclaimContent(' + cid + ',\'' + type + '\',' + owner_id + ',' + id + ',\'' + hash + '\');">�������</a>';
    }
  });
},
unclaimContent: function(cid, type, owner_id, id, hash) {
  ...
  ajax.post('/claims', {act: 'a_unclaim', claim_id: cid, type: type, id: id, owner_id: owner_id, hash: hash}, {
    onDone: function(response) {
      ge('claim' + cid + type + owner_id + '_' + id).innerHTML = '<a href="#" onclick="Claims.claimContent(' + cid + ',\'' + type + '\',' + owner_id + ',' + id + ',\'' + hash + '\');">������</a>';
    }
  });
},
...
checkClaiemdTitle: function(title, el) {
  ajax.post('/claims', {act: 'a_search_blacklist', title: title, hash: cur.hash}, {
    onDone: function(res) {
      if (res) {
...
deleteClaimedTitle: function(title_id) {
  var obj = cur.titles[title_id], title = obj && obj.title || '';
  if (!title) {
    var row = ge('row'+title_id), titleEl = row && geByClass1('claim_title', row);
    if (titleEl) {
...
_videoHashesHideRow: function(ref) {
  ...
  setStyle(el, {opacity:0, height: 0});
  setTimeout(function() {
    re(el);
  }, 200);
},
