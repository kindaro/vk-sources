...
show: function(opts) {
  ...
  autosizeSetup(cur.pbField, {minHeight: 80})
  ...
  Wall.initComposer(cur.pbField, {
    ...
    var draft = opts.draft, composer = data(cur.pbField, 'composer'), note = cur.pbNoteAdded;
  ...
  wkcur._hide.push(function() {
    ...
    if ((composer || {}).addMedia) {
      composer.addMedia.onChange = function() { return false; };
    }
...
send: function() {
  ...
  var toClub = isVisible('pb_choose_club'), hash = cur.pbMyHash, toClubId = 0;
  ...
  ajax.post('al_wall.php', Wall.fixPostParams(extend(params, {
    ...
    onFail: function(text) {
      ...
      setTimeout(animate.pbind(cur.pbError, {backgroundColor: '#FCEBA7'}, 100, animate.pbind(cur.pbError, {backgroundColor: '#F4EBBD'}, 2000)), 0);
      return true;
    },
...
saveDraft: function() {
  ...
  var composer = cur.pbField && data(cur.pbField, 'composer'),
      params = composer ? Composer.getSendParams(composer, PostBox.postChanged.pbind(10), true) : {message: trim(val(cur.pbField))};
  if (params.delayed) return;

  ajax.post('al_wall.php', Wall.fixPostParams(extend(params, {act: 'save_draft', hash: cur.pbMyHash})), {
    onDone: function() {
      clearTimeout(cur.pbSavedTO);
      show(cur.pbSaved);
      animate(cur.pbSaved, {opacity: 1}, 100);
      cur.pbSavedTO = setTimeout(animate.pbind(cur.pbSaved, {opacity: 0}, 1000, hide.pbind(cur.pbSaved)), 3000);
    }, onFail: function() {
      return true;
    }
  });
...
showClubs: function() {
  ...
  WideDropdown.init('pb_club_dd', {
    ...
    onChange: function(act) {
      var dd = cur.wdd['pb_club_dd'], sel = dd.selCount, peer = false, draft, ret = true;
      if (act == 1) { // added
        setTimeout(elfocus.pbind(cur.pbField), 0);
      }
      return ret;
...
try{stManager.done('postbox.js');}catch(e){}
