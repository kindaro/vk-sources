var Print = {
  init: function(items, selected) {
    ...
    var relWrapEl = ge('print_table_wrap_rel'), disablingOverlay = ge('print_disabling_overlay'), loadingOverlay = ge('print_loading_overlay'), emptyOverlay = ge('print_empty_overlay'), tableEl = ge('print_table'), contentEl = ge('print_content');
    var tableWidth = tableEl.offsetWidth, tableHeight = tableEl.offsetHeight;
    ...
    if (cur.drawingSupported) {
      this.preloadFonts(function() {
        ...
        animate(loadingOverlay, {opacity: 0}, this.anim.hideLoading, function() {
          re(loadingOverlay);
        });
      }.bind(this), function() {
        //cur.fontsPreloaded = true;
  ...
  initFallback: function() {
    ...
    removeClass(ge('print_dd'), 'disabled');
    ge('print_dd_input').removeAttribute('disabled');
    ge('print_mockups_header').innerHTML = clean(getLang('print_stickers_example'));
  },
  destroy: function() {
  ...
  groupSelected: function(group) {
    var id = intval(group[0]), is_group = id < 0, domain = group[4].substring(1);

    var emptyEl = ge('print_empty'), emptyAbsEl = ge('print_empty_abs'), dlWrapEl = ge('print_dl_wrap'), disablingOverlay = ge('print_disabling_overlay'), emptyOverlay = ge('print_empty_overlay');
    if (domain.match(/^(club|event|public|id)[\d]+$/)) {
      ...
      var emptyTextEl = ge('print_empty_text'), emptyForAdminsEl = ge('print_empty_for_admins'), emptyForAdminsLink = ge('print_empty_for_admins_link');
      ...
      var totalHeight = emptyOverlay.offsetHeight, blockHeight = emptyEl.offsetHeight;
      setStyle(emptyAbsEl, {
    ...
    if (!cur.urls[domain]) {
      cur.urlsRequest = ajax.post('/al_print.php', {act: 'get_urls', id: id, typo: (cur.typo ? 1 : 0)}, {
  ...
  groupRemoved: function() {
    var infoEl = ge('print_msg_wrap'), dlWrapEl = ge('print_dl_wrap'), disablingOverlay = ge('print_disabling_overlay'), emptyOverlay = ge('print_empty_overlay');
  ...
  download: function(type, format) {
    ...
    var url = cur.urls[cur.domain][type][format];
    if (!url) {
      return false;
    }

    var iframe = ce('iframe', {src: url});
  ...
  downloadAll: function() {
    if (!cur.urls[cur.domain] || !cur.urls[cur.domain].zip) return false;

    var url = cur.urls[cur.domain].zip;
  ...
  showProgress: function() {
    var button = ge('print_dl_button_wrap'), progress = ge('print_dl_progress_abs'), done = ge('print_dl_done_abs');

    setStyle(progress, 'visibility', 'visible');
    animate(progress, {opacity: 1}, this.anim.progress);
    animate(button, {opacity: 0}, this.anim.progress, function() {
      setStyle(button, {visibility: 'hidden'});
    });
  ...
  showProgressDone: function() {
    ...
    animate(done, {opacity: 1}, this.anim.progress);
    animate(progress, {opacity: 0}, this.anim.progress, function() {
      setStyle(progress, {visibility: 'hidden'});
    });
  ...
  hideProgress: function() {
    ...
    animate(button, {opacity: 1}, this.anim.progress);
    animate(done, {opacity: 0}, this.anim.progress, function() {
      setStyle(done, {visibility: 'hidden'});
    });
    ...
    setStyle(ge('print_dl_progress_fill'), {width: '0px'});
  ...
  getProgress: function() {
    ...
    var interval = 750, start = new Date().getTime();
    cur.progressRequest = ajax.plainpost(cur.urls[cur.domain].zip_progress, {}, function(data) {
      if (cur.ignoreProgress) {
        this.hideProgress();
        return;
      }
      ...
      data = parseJSON(data);
      var time = new Date().getTime(), val = floatval(data.progress);
      if (val >= 100) {
  ...
  setProgress: function(val) {
    if (val <= 0) return;
    var total = ge('print_dl_progress').offsetWidth, cur = total / 100 * val;
    animate(ge('print_dl_progress_fill'), {width: cur, overflow: 'visible'}, this.anim.progress);
  },
  ...
  drawPreviews: function() {
    ...
    var horizTd = geByClass1('print_preview_horiz'), horizWidth = horizTd.clientWidth - intval(getStyle(horizTd, 'padding-left')) - intval(getStyle(horizTd, 'padding-right'));

    each (this.types, function(type, config) {
      var isHoriz = (type != 'qr');
      var cnv = ge('print_canvas_'+type), cnvHolder = ge('print_canvas_holder_'+type), ctx, cnvSize;
      var scale = 1;
      ...
      cnvSize = {width: cnv.width, height: cnv.height};
      ...
      if (type == 'qr') {
        ...
        var fontLine = this.fontLine.replace('{size}', fontSize + 'px'), boldFontLine = this.boldFontLine.replace('{size}', fontSize + 'px');

        ctx.textBaseline = 'alphabetic';
        ctx.fillStyle = config.textColor;

        ctx.font = fontLine;
        ctx.fillText(this.prefix, 0,
    ...
    var address = "http://vk.com/" + domain;
    if (!cur.qrcode) {
      cur.qrcode = new QRCode(ge('print_qrcode'), {
        ...
        vkLogo: {
          sizes: {
            29: {width: 11, height: 8},
            33: {width: 13, height: 9},
            37: {width: 13, height: 10},
            41: {width: 15, height: 11}
          }
        }
      });
    } else {
  ...
  calcQrFontSize: function(text, width, max_height) {
    ...
    var tc = ce('canvas'), tcctx = tc.getContext('2d');
    var i = max, size = i + plus, w, half = i, iters = 0;
  ...
  measureTextWidths: function(domain, fontSize) {
    var tc = ce('canvas'), tcctx = tc.getContext('2d'), vals = {};

    tcctx.font = this.fontLine.replace('{size}', fontSize + 'px');
    vals.prefix = tcctx.measureText(this.prefix).width;
  ...
  showOverlay: function(el, opacity) {
    ...
    show(el);
    animate(el, {opacity: opacity}, this.anim.toggle);
  },
  hideOverlay: function(el) {
    animate(el, {opacity: 0}, this.anim.toggle, function() {
      hide(el);
    });
  },
...
function loadFontFace(fontFamily, options) {
  ...
  function load() {
    ...
    sansSerif = ce('div', {innerHTML: testString});
    serif = ce('div', {innerHTML: testString});

    sansSerif.setAttribute('style', getStyleString(extend({'font-family': 'sans-serif'}, style)));
    serif.setAttribute('style', getStyleString(extend({'font-family': 'serif'}, style)));
...
try{stManager.done('print.js');}catch(e){}
