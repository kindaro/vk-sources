...
onSortEnd = function(elem) {
  ...
  var ajax = new Ajax();
  ajax.onDone = function() { hide('progr'); };
  var gidid = elem.innerHTML.match(/GID(\d+)ID(\d+)/);
  if (startPos != newPos && newPos >= 0 && gidid[1] && gidid[2]) {
    show('progr');
    ajax.post('/groups_ajax.php', {act: ge('reorderact').value, id: gidid[2], gid: gidid[1], oldpos: startPos, newpos: newPos});		
  }
};
...
function showEditOfficerBox(user_id, group_id) {
  ...
  showBoxOld(function() { 
    if (ge('position').value.length > 0) {
      ...
      ajax.onDone = function() { ge('pos'+ user_id).innerHTML = ', '+ position; ge('position'+ user_id).value = position; ge('is_admin'+ user_id).value = admin2; hideBoxOld(); };
      ajax.post('/groups_ajax.php', {'act': 'aeditofficer', 'id': user_id, 'gid': group_id, 'position': position, 'admin': admin});
...
function showDeleteOfficerBox(user_id, group_id) {
  ...
  showBoxOld(function() {
    var ajax = new Ajax();
    ge('boxMessage').innerHTML = '<div style="text-align: center"><img valign="middle" src="/images/progress7.gif"></div>';			
    ajax.onDone = function() { ge('officer'+ user_id).parentNode.removeChild(ge('officer'+ user_id)); hideBoxOld(); };
    ajax.post('/groups_ajax.php', {'act': 'adeleteofficer', 'id': user_id, 'gid': group_id});
  });
  return false;
}

function showEditLinkBox(link_id, group_id) {
  var mb = new MessageBox({title:groups_editing_link});
  mb.addButton({label:groups_cancel, style:'button_no', onClick:function(){mb.hide();}});
  mb.addButton({label:groups_edit, onClick: function() {
    if (ge('position').value.length > 0) {
      var position = ge('position').value;
      Ajax.Post({url:'/groups_ajax.php', query:{'act': 'aeditlink', 'id': link_id, 'gid': group_id, 'position': position}, onDone:function() { 
...
function showDeleteLinkBox(link_id, group_id) {
  var mb = new MessageBox({title:groups_deleting_link});
  mb.addButton({label:groups_no, style:'button_no', onClick:function(){mb.hide();}});
  mb.addButton({label:groups_yes, onClick:function(){
    Ajax.Post({url:'/groups_ajax.php', query:{'act': 'adeletelink', 'id': link_id, 'gid': group_id}, onDone:function(){
...
function doAjax(uid, gid, act, message, fadefunc) {
  ...
  ajax.onDone = function() { 
    ...
    setTimeout(function() { new effects.fader().fade(ge('memberRow_'+ uid), [0xFFFFFF, 0xD8DFEA], 3000); }, 2500);
  };
  ge('memberOptions_'+ uid).innerHTML = '<img style="overflow: hidden;" valign="middle" src="/images/upload_mini.gif"> ';
  ajax.post('/groups_ajax.php', {'act': act, 'id': uid, 'gid': gid});
  return false;	
}
    ...
    if (ge('position').value.length > 0) { 
      ...
      ajax.onDone = function() {
        switch (success_style) {
          ...
          case 1:
          default:
            hideBoxOld(function() { location.href = '/groups.php?act=officers&gid='+ group_id +'&msg=1'; });
        }	
      }
      ajax.post('/groups_ajax.php', { act: 'aappoint', id: user_id, gid: group_id, hash: hash, position: ge('position').value, admin: ge('admin').checked ? 1 : 0 });
  ...
  if (window.$ah && $ah.onLoad && $ah.onLoad.pages) {
    ...
    if (matches) {
      ...
      $ah.onLoad['pages'].show = {
        to: function(p) { return p.offset+"_"+p.op; },
        from: function(p) {
          var o = p.split("_");
          if (!o[1]) o[1] = defOp;
          return {act:'getpages',gid:window.group_id,op:parseInt(o[1]),offset:parseInt(o[0])};
        }
      }
      $ah.useCache = false;
    }
...
function switchTrigger(op){
  ...
  getPageContent(0, 1, null, {act:'getpages',gid:window.group_id,op:op});
}

function approvePage(){
  var memDiv = ge('memberPages');
  var group_id = parseInt(document.inviteFriends.gid.value);
  var params = {act:'aapprove', gid:group_id};
  var n = 1;
...
function inviteFriends(){
  var group_id = parseInt(document.inviteFriends.gid.value);
  params = {act:'ainvitefriends', gid:group_id};
  var inputs = document.inviteFriends.getElementsByTagName("input");
  for(var i = 0; i < inputs.length; i++){
    if (parseInt(inputs[i].name)) params[inputs[i].name] = 1;
  }

  Ajax.postWithCaptcha('groups_ajax.php', params, {onSuccess: function (ajaxObj, responseText) {
...
function removeSearch(item_id, section, oid) {
  ...
  Ajax.Post({url:'/gsearch.php', query:{act:'a_remove_search', section:section, item_id:item_id, oid:oid},
...
function restoreSearch(item_id, section, oid) {
  ...
  if (!el) return;
  var html = el.innerHTML;
  Ajax.Post({url:'/gsearch.php', query:{act:'a_restore_search', section:section, item_id:item_id, oid:oid},
    onDone:function(res, text){
      el.innerHTML = text;
    },
    onFail:function(res, text) {
...
function addFaveGroup(gid, hash) {
  ...
  Ajax.Send('/fave.php', {'act':'a_add_group', 'gid': gid, hash: hash}, function(o,t){
...
function deleteFaveGroup(gid, hash) {
  ge('faveProgress').innerHTML = '<IMG SRC="images/upload.gif">';
  Ajax.Send('/fave.php', {'act':'a_delete_group', 'gid': gid, hash: hash}, function(o,t){
    ge('addToFaves').innerHTML = t;
  });
}
...
function enterGroup(gid, hash) {
  if (entering) return;
  entering = true;
  var stop = function() { entering = false; };
  Ajax.postWithCaptcha('/groups_ajax.php', {act: 'a_enter', gid: gid, hash: hash}, {onSuccess: function(obj, text) {
    entering = false;
    if (text.substr(0, 1) == 'r') {
      location.replace(text.substr(1));
    }
  }, onFail: stop, onCaptchaShow: stop});
}
...
function leaveGroup(gid, hash, text, enterHash) {
  ...
  if (obj) {
    ...
    Ajax.postWithCaptcha('/al_public.php', {act: 'a_leave', al: 1, pid: gid, hash: hash}, {onSuccess: function(obj, text) {
...
function postGroupStatus(to_id, hash) {
  ...
  var l = ge('add_wall_media_link_status'), f = data(l, 'postFunc');
  if (f) {
    f();
    return;
  }
  ...
  var params = {act: 'a_post_wall', hash: decodehash(hash), message: txt, to_id: -to_id, reply_to: reply_to};
...
function setupReply() {
  each(geByClass('reply_message'), function(i,v) {
    placeholderSetup(v);
    if (!v.autosize) {
      v.autosize = new AutosizeMod(v, {minHeight:30});
    }
  });
}
...
function postWall(to_id, hash, top_id, type) {
  ...
  var l = ge('add_wall_media_link'), f = data(l, 'postFunc');
  if (f) {
    f();
    return;
  }
  ...
  var params = {act: 'a_post_wall', hash: decodehash(hash), message: txt, to_id: to_id, reply_to: (ge('reply_to' + full_id) || {}).value, top_id: top_id, type: type};
  if (full_id) {
...
function showGroupStatusHistory(gid, type, hash, offset) {
  Ajax.Send('wall.php', {act:'a_get_wall', type:type, to_id:-gid, hash:hash, offset: offset}, function(o,t) {
    var res = eval('('+t+')');
...
function highlightReply(el) {
  var e = ge(el);
  if (!e) return;
  animate(e, {backgroundColor: '#ECEFF3'}, 200, function() {
    setTimeout(function() {
      animate(e, {backgroundColor: '#FFF'}, 200);
    }, 1000);
  });
}
function showReply(to_id, reply_id, msg_id) {
  var k = 'wall_reply' + to_id + '_' + reply_id, r = ge(k);
  if (r) {
    highlightReply(r);
  } else {
...
function showReplies(to_id, msg_id, count, hl) {
  Ajax.Send('wall.php', {act: 'a_get_replies', msg_id: msg_id, to_id: to_id, count: count}, function(o, t) {
...
function toWallComments(to_id, wall_hash, status_hash) {
  ...
  if (!wallHistory[1]) {
    Ajax.Send('wall.php', {act:'a_get_wall', type:1, to_id:-to_id, hash:wall_hash, offset: 0}, function(o,t) {
      var res = eval('('+t+')');
...
function toStatusBox(to_id, wall_hash, status_hash) {
  ...
  if (!wallHistory[0]) {
    Ajax.Send('wall.php', {act:'a_get_wall', type:2, to_id:-to_id, hash:status_hash, offset: 0}, function(o,t) {
      var res = eval('('+t+')');
...
function deleteStatus(cid, oid, hash) {
  ...
  Ajax.Send('/wall.php', {act: 'a_delete', oid: oid, cid: cid, hash: hash}, 
    function(o, t) {
...
onDomReady(function() {
  ...
  if (el) {
    el.value = '';
    placeholderSetup(el);
    el.autosize = new AutosizeMod(el, {minHeight:30});
...
function officersInGroup(gid, tabbed) {
  var mb;
  if (tabbed) {
    mb = new TabbedBox('officers', [['officers', getLang('groups_officers')], ['admins', getLang('groups_admins')]], {
    title: getLang('groups_officers_of_group'),
      tabStyle: 'height: 310px;',
      width: 478,
      onTab: function(tab, loaded) {
        if (!loaded) {
          mb.tabLoadContent(tab, '/groups.php', {act: 'a_get_officers_in_group', tab: tab, gid: gid}, false, 'height: 300px;').show();
          return false;
        }
      }
    });
    ...
    mb.loadContent('/groups.php', {act: 'a_get_officers_in_group', tab: 'admins', gid: gid}, false, 'height:300px;').show();
  }
}
