...
function showMessage(params) {
  var defaults = {title: '������', text: '��������� ������', close: '�������'};
  params = extend(defaults, params);
  if (!msgBox) {
    msgBox = new MessageBox({returnHidden: true});
    msgBox.addButton({label: params.close, style: 'button_no', onClick: function() { msgBox.hide(); }});
  }
  msgBox.setOptions({title: params.title});
...
function addMainTopic(page) {
  ...
  ajax.post('support.php', {l: lang_id, act: 'a_add_main_topic', page: page, hash: ge('add_main_topic_hash').value});
}
...
function removeMainTopic(id) {
  ...
  show('edit_main_topic_progress');
  ajax.post('support.php', {l: lang_id, act: 'a_remove_main_topic', topic_id: id, hash: ge('main_topic_hash' + id).value});
}
...
function restoreMainTopic(id) {
  ...
  ajax.onFail = failFunction('edit_main_topic_progress');
  show('edit_main_topic_progress');
  ajax.post('support.php', {l: lang_id, act: 'a_restore_main_topic', topic_id: id, hash: ge('main_topic_hash' + id).value});
}

function initMoveTopicSections(index, parent_id) {
  var container = ge('section_select'), elem = null;
  ...
  ajax.onDone = doneFunction('section_progress' + index, function(response) {
    if (!response.error) {
      ...
      sections_json.sort(function(a, b) { return a[1] > b[1] ? 1 : (a[1] < b[1] ? -1 : 0); });
      new Dropdown(ge('section' + index), sections_json, {onChange: function(value) {
  ...
  ajax.post('support.php', {l: lang_id, act: 'a_get_sections', parent_id: parent_id});
}

var moveTopicBox = null;
function showMoveTopicBox(id) {
  if (!moveTopicBox) {
    moveTopicBox = new MessageBox({title: '����������� �������'});
    moveTopicBox.addButton({label: "�������", style: "button_no", onClick: function() { moveTopicBox.hide(); }});
  }  
  ...
  ajax.onFail = failFunction('box_loader', function() { moveTopicBox.hide(); });
  ajax.post('support.php', {l: lang_id, act: 'a_move_topic_box', topic_id: id});
}
...
function moveTopic(parent_id) {
  ...
  ajax.onFail = failFunction('box_loader', function() { moveTopicBox.hide(); });
  ajax.post('support.php', {l: lang_id, act: 'a_move_topic', topic_id: topic_id, section_id: parent_id, hash: ge('move_topic_hash').value});
  moveTopicBox.content('<div id="box_loader" class="box_loader"></div>');
}

function startNotifications() {
  new Checkbox(ge('notify'), {label: '��������� ���������', checked: ge('notify').value, width: 150, onChange: function() {
    (new Ajax()).post('support.php', {l: lang_id, act: 'a_change_notify', notify: ge('notify').value});
  }});
}
