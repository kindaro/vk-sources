(function(){
var translationBox, menuBox, saveInProgress = false, inviteHash;
...
window.t = function(event, el, leftBtn, callback, returnHidden, deleteHash) {
  ...
  transEl = el;
  if (!translationBox) {
    translationBox = new MessageBox({title: getLang(deleteHash != false ? 'tran_change_title' : 'tran_restore_title'), width: 490, progress: 'transProgress'/*, onLoad: updateBox*/, returnHidden:returnHidden});
  }
  if(createBox){createBox.content('');}
  var langKey = (el.id.substr(0,5) == 'lang_') ? el.id.substr(5) : el.id;
  hide('transProgress');
  translationBox.removeButtons().addButton({
    label: getLang('box_cancel'), style: 'button_gray', onClick: function(){translationBox.hide(200);saveInProgress = false;}
  });
  if (deleteHash == false)  {
    translationBox.addButton({
      label: getLang('box_restore'), onClick: function(){doRestoreKey(transEl)}
    }).loadContent('translation.php', {act:'a_deleted_box',key:langKey}, true).show();
  } else {
    //if (deleteHash) {
    ...
    //}
    translationBox.addButton({
      label: getLang('box_save'), onClick: window.tDoSave = function(){doTranslate(transEl, callback)}
    }).loadContent('translation.php', {act:'inline',key:langKey}, true).show();
  }
  cancelEvent(event);
  return false;
}
function doTranslate(translatedEl, callback) {
  ...
  Ajax.Post({
    ...
    onFail: function(ajaxObj, responseText) {
      ...
      try {var res = eval('('+responseText+')'); text = res.error || text;} catch(e){}
...
window.doCloneKey = function(curKey, newSectionId, sectionId, hash, isMove) {
  ...
  Ajax.Post({
    url: 'translation.php',
    query: { act: 'key_clone', key: curKey, new_key: newKey, new_section_id: newSectionId, move: isMove, section_id: sectionId, hash: hash },
    onDone: function(ajaxObj, responseText) {
...
window.doDeleteKey = function(key, hash) {
  ...
  AlertBox('������������� ��������', getLang('tran_delete_key_confirm_text'), function() {
    saveInProgress = true;
    var query = {act: 'a_delete_key', key: key, section_id: ge('translationForm')['section_id'].value, hash: hash};
    ...
    //show('transProgress');
    //hide('transDeleteLink');
  }, {boxType: 'CONFIRM', returnHidden: true}).show();
  return false;
}
...
window.updateBox = function() {
  var nativeText = ge('nativeText'), history = ge('historyWrap');
  var items = ge('translationForm').getElementsByTagName('textarea'), tokens;
...
window.createLangKey = function(section_id) {
  if (!createBox) {
    createBox = new MessageBox({title: getLang('tran_create_new_title'), width: 490, progress: 'createProgress', onLoad: updateCreateBox});
  }
  hide('createProgress');
  if(translationBox){translationBox.content('');}
  createBox.removeButtons().addButton({
    label: getLang('box_cancel'), style: 'button_gray', onClick: function(){createBox.hide(200);saveInProgress = false;}
  }).addButton({
    label: getLang('tran_box_create'), onClick: doCreateKey
  }).loadContent('translation.php', {act:'inline_create', section_id: section_id}, true).show();
  return false;
}
  ...
  Ajax.Send(
    ...
    function(ajaxObj, responseText) {
      ...
      if (result.result){
        ...
        createBox.removeButtons().addButton({label: getLang('box_close'), style: 'button_gray', onClick: function() {createBox.hide(200);}});
        setTimeout(function(){createBox.hide(200);}, 1200);
...
window.inlineMenu = function(sections, admin_href, admin_name, hash) {
  ...
  if (!menuBox) {
    menuBox = new MessageBox({title: 'Select option',  width:'200px', bodyStyle: 'padding:0px'});
    menuBox.addButton({label: 'Close', style: 'button_gray', onClick: function(){
      menuBox.hide();
    }});
  ...
  if (!uiInviteFriend) {
    ...
    jsDispatcher.include('lib/ui_controls', function() {
      Ajax.Post({
        ...
        query: {from:'inline', filter:'tiny'},
        onDone: function(ajaxObj, responseText){
          var result = eval('('+responseText+')');
          hide('friendsProgress');
          if (result.friends && result.friends.length) {
            uiInviteFriend = Dropdown(ge('inviteFriendId'), result.friends, {width: 150, autocomplete:true, placeholder:'Start typing friend\'s name'});
            show('inviteControls');
          } else {
...
window.doInvite = function() {
  ...
  Ajax.Post({
    url: 'translation.php',
    query: {act:'a_invite_translator', user_id: friendId, hash: inviteHash},
    onDone: function(ajaxObj, responseText) {
      var result = eval('('+responseText+')'), invitationText = ge('invitationText'), text = '';
      if (result.result) {
        text = result.result;
      } else if (result.error) {
        text = result.error;
...
window.showAllPhrases = function(sections) {
  ...
  Ajax.Post({
   url: 'translation.php',
   query: {act: 'inline_edit_all', sections: sections},
   onDone: function(ajaxObj, responseText) {
  ...
  animate(document.getElementsByTagName('html')[0], {scrollTop: coords[1]});
  animate(document.getElementsByTagName('body')[0], {scrollTop: coords[1]});
...
window.hideAllPhrases = function() {
  ...
  hide(elAllPhrases);
  show('banner1');
  animate(document.getElementsByTagName('html')[0], {scrollTop: 0});
  animate(document.getElementsByTagName('body')[0], {scrollTop: 0});
  return false;
}
...
window.batchSearchKey = function(force) {
  ...
  Ajax.Post({
    url: 'translation.php',
    query: { act: 'batch_edit_search_key', key: key },
    onDone: function(ajaxObj, responseText) {
