...
var http_request = false; if (window.XMLHttpRequest) {http_request = new XMLHttpRequest();} else if (window.ActiveXObject) { try {http_request = new ActiveXObject("Msxml2.XMLHTTP");} catch (e) {try {http_request = new ActiveXObject("Microsoft.XMLHTTP");} catch (e) {}}} if (!http_request) {alert('�訡�� ��� ᮧ����� XMLHTTP'); return false;}return http_request;}
...
function delete_all_messages(box_title, box_text, peer_id, hash) {
  if (!window.deleteAllMessagesBox) {
    window.deleteAllMessagesBox = new MessageBox({width:400, bodyStyle:'padding: 10px', closeButton:false, progress:'deleteBoxProgress'});
  }
  window.deleteAllMessagesBox.removeButtons();
  window.deleteAllMessagesBox.addButton({label:global_cancel, style:'button_no', onClick:function() {
    window.deleteAllMessagesBox.hide();
  }});
  window.deleteAllMessagesBox.addButton({label:global_delete, onClick:function() {
    ...
    ajax.post('/mail.php', {'act':'a_delete_all', 'peer_id':peer_id, 'hash':hash});
  }});

  window.deleteAllMessagesBox.setOptions({title:box_title});
...
function delete_message(type, id)
{
  ...
  ajax.post('/mail.php', {'act': 'a_delete', 'id': id, 'out': outbox});
}
...
function restore_message(type, id)
{
  ...
  ajax.post('/mail.php', {'act': 'a_restore', 'id': id, 'out': outbox});
}
