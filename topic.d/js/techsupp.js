...
function load_topics_page() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_get_topics_page', 'page': index, 'perpage': 30, 'section_id': params['section_id'], 'p': params['path']});
}
...
function load_search_page() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_search_page', 'page': index, 'perpage': params['per_page'], 'q': ge('query').innerHTML});
}
...
function load_na_nc_page() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_na_nc_page', 'page': index, 'perpage': params['per_page'], 'not_answered': ge('not_answered').innerHTML});
}
...
function load_na_filtered_page() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_na_filtered_page', 'page': index, 'perpage': params['per_page']});
}
...
function load_users_topics_page() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_users_topics_page', 'page': index, 'perpage': params['per_page'], 'mid': ge('member_id').innerHTML});
}
...
function load_page()
{
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_get_page', 'tid': parseInt(params['topic_id']), 'page': index, 'perpage': params['per_page']});
}

function update_page_links()
{
  var cur_page = parseInt(params['page']), last_page = parseInt(params['pages_count']);
...
function load_similar() {
  if (trim(ge('title').value).length) {
    ...
    ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_search_page', 'page': '1', 'perpage': '20', 'q': ge('title').value});
  } else {
...
function check_filter(section_id) {
  ...
  var filtered = ge('forum_filter').innerHTML.split(','), new_filtered = new Array();
...
function save_filter() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_update_filter', 'filter': ge('forum_filter').innerHTML});
}
///////////////////////////////////////// Filtering not answered questions by sections }
///////////////////////////////////////// Ban user {
...
function banlist_page(page) {
  ...
  ajax.post('/' + url, {'act': 'a_banlist_page', 'fid': params['forum_id'], 'index': page});
...
function ban_user() {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_ban_user', 'page': ge('page').value, 'comment': ge('comment').value, 'hash': ge('hash').value, 'do_delete': (ge('do_delete').checked ? 1 : 0)});
}
...
function unban_user(index) {
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_remove_ban', 'user_id': index, 'hash': ge('hash' + index).value});
}
...
function show_details(id) { // In section
  ...
  ajax.post('/' + url, {'fid': params['forum_id'], 'act': 'a_topic_details', 'tid': id});
...
function showAutoAnswersBox(tid) {
  ...
  ajax.post('/techsupp.php', {'act': 'a_get_autoanswers', 'fid': 1});
...
function addAutoAnswer() {
  ...
  if (answer_name && answer_text) {
    ajax.post('/techsupp.php', {'act': 'a_add_autoanswer', 'fid': 1, 'name': answer_name, 'text': answer_text});
    ge('answersTable').innerHTML = progressLine;
  } else {
    alert("������� �������� � ����� ������.");
  }
...
function showAutoAnswer(answer_id, section) {
  ...
  //ajax.onFail = opFailed;
  ajax.post('/techsupp.php', {'act': 'a_get_autoanswer', 'fid': 1, 'answer_id': answer_id, 'section': section});
}
...
function deleteAutoAnswer(answer_id, section) {
  ...
  ajax.onDone = getAutoAnswersSuccess;
  //ajax.onFail = opFailed;
  ajax.post('/techsupp.php', {'act': 'a_delete_autoanswer', 'fid': 1, 'answer_id': answer_id, 'section': section});
  ge('answersTable').innerHTML = progressLine;
}
...
function can_be_notified(value) {
  var ajax = new Ajax();
  ajax.post('/support.php', {l: ge('forum_id').innerHTML, act: 'a_change_notify', notify: value ? 1 : 0});
  if (value) {
...
function notify() {
  ...
  if (users.length) {
    ...
    ajax.post('support.php', {l: ge('forum_id').innerHTML, act: 'a_notify', topic_id: ge('topic_id').innerHTML, users: users_str});
  }
}