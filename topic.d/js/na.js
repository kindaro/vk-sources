function change_mail() {
  showBox('change_mail', 'register.php', {act: 'change_mail_box'}, false, true, {type: 'POPUP', hideOnClick: false, progress: 'change_progress'}, ['css/al/reg.css']);
}
...
function do_change_mail(use_old_mail) {
  ...
  if (use_old_mail !== 1) {
    ...
    if (!resend_box) {
      resend_box = new MessageBox({hideOnClick: false});
    }
    resend_box.setOptions({type: 'POPUP'});
    resend_box.content('<div class="box_loader"></div>').show();
  }
  Ajax.postWithCaptcha('register.php', {act: 'do_change_mail', email: mail}, {onSuccess: function(obj, text) {
    ...
    if (response.error) {
      if (window.winBoxes.change_mail) {
        ...
        resend_box.setOptions({type: 'MESSAGE', title: getLang('global_error')});
        resend_box.addButton({label: getLang('box_close'), onClick: resend_box.hide});
      ...
      if (window.winBoxes.change_mail) {
        ...
        window.winBoxes.change_mail.removeButtons().addButton({label: getLang('box_close'), onClick: window.winBoxes.change_mail.hide});
        setTimeout(function() { window.winBoxes.change_mail.hide(500); }, 2000);
      } else if (resend_box) {
        resend_box.setOptions({type: 'MESSAGE', title: response.title});
        resend_box.addButton({label: getLang('box_close'), onClick: resend_box.hide});
        resend_box.content(response.txt);
        setTimeout(function() { resend_box.hide(500); }, 2000);
      }
...
function show_change_mail_box(on_hide) {
  if (!window._please_box) {
    window._please_box = new MessageBox({title: ge('please_confirm_title').value});
    window._please_box.addButton({label: getLang('box_close'), onClick: window._please_box.hide});
    window._please_box.content(ge('please_confirm_mail').innerHTML);
  }
  window._please_box.setOptions({onHide: on_hide});
